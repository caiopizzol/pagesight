import { startRenderNetwork } from "../web/render-network.js";
...
export async function verifyRender(options: RenderOptions) {
  return capture("pagesight", "page.verify", options.url, options, async () => {
    const network = await startRenderNetwork(options.url);
    try {
      const browser = await launchRenderer(network.proxy);
      try {
        ...
        return {
          ...
          network: {
            ...
            policy:
              "Public addresses only; an explicitly requested literal private IP permits that exact host and port.",
          },
          ...
          documentComparison: (() => {
            ...
            if (!a || !b)
              return {
                status: "unavailable",
                reason: "Both server and target browser document responses are required.",
              };
    ...
    const data = result.pages[0]?.response as
      | {
          observations: Record<
            string,
            { status: string; pages: Array<{ response: { dom?: SeoDom; documentsTruncated?: boolean } }> } | null
          >;
    ...
    if (
      data &&
      (Object.values(data.observations).some(
        (e) =>
          e && (e.status !== "ok" || e.pages[0]?.response.dom?.truncated || e.pages[0]?.response.documentsTruncated),
      ) ||
