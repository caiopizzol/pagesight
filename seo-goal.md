...
PR #8 Cubic finding reproduced: Google's documented `nthYear` is relative to each
date range, and a dimension expression can alias it under a non-time name. Both
previously produced deltas. GA comparison now accepts only the six dimensions
generated by snapshots (or no dimensions), without dimension expressions; GSC
continues to reject date/hour keys. README and PRODUCT.md state the restricted
contract. Regressions reject both temporal cases and retain all five GA snapshot
report variants. All 93 Bun tests, TypeScript 7 and Vite+ pass, with four unchanged
legacy warnings. This review fix awaits publication and thread resolution.
